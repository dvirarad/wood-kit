...
cd backend
npm test -- --testPathIgnorePatterns=complete-product-flow --silent
UNIT_TEST_EXIT=$?
cd ..
...
cd frontend
REACT_APP_API_URL=http://localhost:6003/api/v1 npm run build --silent >/dev/null 2>&1
BUILD_EXIT=$?
cd ..
...
# Start backend server for E2E test
echo -e "${BLUE}🔧 Starting backend server for E2E test...${NC}"
cd backend
...
cd backend
npx jest __tests__/complete-product-flow.test.js --verbose --detectOpenHandles --forceExit --testTimeout=60000 --runInBand --silent
E2E_EXIT_CODE=$?
cd ..
...
if [ $E2E_EXIT_CODE -eq 0 ]; then
    ...
    echo -e "${GREEN}✅ E2E test suite: PASSED (15/15)${NC}"
    echo ""
